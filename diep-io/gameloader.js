...
try {
    ...
    function findServer() {
		if (!killedTab) {
			...
			let lastFfaID = localStorage.getItem("lastFfaId") || 0

			setTimeout(() => {
				connectToIP()
				g("precon-ffa-" + lastFfaID).click()
			}, 1000)
		}
    };
    ...
    function connectToIP() {
    	...
    	// Should refresh the page on connection time out. - berkant
    ...
    function serverTimeout() {
    	if (!serverFound) {
    		// findServer();
			localStorage.setItem('lastFfaId', '0') // fall back - berkant
			location.reload()
    	}
    };
